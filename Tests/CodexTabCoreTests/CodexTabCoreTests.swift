...
private enum CodexTabTestError: Error {
    case tailMissing
...
private func spawnTailProcess(url: URL) throws -> Process {
    guard let tailURL = tailExecutableURL() else {
        throw CodexTabTestError.tailMissing
    }
    let process = Process()
...
final class CodexTabCoreTests: XCTestCase {
    func testOpenFilePathsIncludesTempFile() throws {
        let (url, handle) = try openTempFile(named: "libproc-\(UUID().uuidString).txt")
        defer {
            try? handle.close()
        ...
        XCTAssertTrue(paths.contains(normalizedPath(url.path)))
    }

    func testOpenRolloutLogsFiltersByName() throws {
        ...
        XCTAssertTrue(paths.contains(normalizedPath(logURL.path)))
        XCTAssertFalse(paths.contains(normalizedPath(otherURL.path)))
    }

    func testOpenFilePathsIncludesChildProcessFile() throws {
    ...
    func testBestLogCandidatePrefersClosestTimestamp() throws {
        ...
        guard let startTime = LogSelection.parseIsoTimestamp(startIso) else {
            throw CodexTabTestError.invalidTimestamp
        }
        ...
        XCTAssertNotNil(metaFirst)
        XCTAssertNotNil(metaSecond)
        ...
        XCTAssertEqual(chosen?.path, first.path)
    }

    func testBestLogCandidatePrefersCwdMatch() throws {
        let tmpdir = FileManager.default.temporaryDirectory.appendingPathComponent(
            "log-selection-\(UUID().uuidString)"
        )
        try FileManager.default.createDirectory(at: tmpdir, withIntermediateDirectories: true)
        ...
        XCTAssertEqual(chosen?.path, second.path)
    }

    func testBestLogCandidateIgnoresSkewedMetaTimestamp() throws {
        let tmpdir = FileManager.default.temporaryDirectory.appendingPathComponent(
            "log-selection-\(UUID().uuidString)"
        )
        try FileManager.default.createDirectory(at: tmpdir, withIntermediateDirectories: true)
    ...
    func testStatusLogPathPrefersTui() throws {
        ...
        XCTAssertEqual(normalizedURLPath(result.path), normalizedPath(logPath.path))
        XCTAssertEqual(result.source, .tui)
    }

    func testStatusLogPathUsesHistory() throws {
        let codexHome = try createCodexHome()
        defer { try? FileManager.default.removeItem(at: codexHome) }
        let sessionsRoot = codexHome.appendingPathComponent("sessions")
        let cwd = codexHome.appendingPathComponent("workdir")
    ...
    func testStatusLogPathUsesSessionDir() throws {
        let codexHome = try createCodexHome()
        defer { try? FileManager.default.removeItem(at: codexHome) }
        let sessionsRoot = codexHome.appendingPathComponent("sessions")
        ...
        XCTAssertEqual(normalizedURLPath(result.path), normalizedPath(newer.path))
        XCTAssertEqual(result.source, .sessionDir)
    }

    func testCodexLogReducerIgnoresBootstrapMessage() {
    ...
    func testCodexLogReducerMarksRunningAndDone() {
        ...
        XCTAssertEqual(done, .doneNoCommit)
    }

    func testCodexLogReducerMarksDoneCommittedOnSuccessfulGitCommit() {
        ...
        XCTAssertEqual(state.title, .doneCommitted)
    }

    func testCodexLogReducerMarksDoneNoCommitOnFailedGitCommit() {
        var state = CodexLogReducer.TurnState(title: .new)
        ...
        XCTAssertEqual(state.title, .doneNoCommit)
    }

    func testTimeoutOverlayActivatesAfterInactivity() {
        var overlay = CodexTimeoutOverlay(now: 0)
        XCTAssertFalse(overlay.timeoutActive)
        ...
        XCTAssertTrue(overlay.timeoutActive)
    }

    func testTimeoutOverlayClearsOnActivity() {
        var overlay = CodexTimeoutOverlay(now: 0)
        ...
        XCTAssertTrue(activatedAgain)
        XCTAssertTrue(overlay.timeoutActive)
    }

    func testTimeoutOverlayIgnoresNonRunningState() {
        var overlay = CodexTimeoutOverlay(now: 0)
    ...
    func testTimeoutOverlayClearsWhenUnderlyingStopsRunning() {
        var overlay = CodexTimeoutOverlay(now: 0)
        _ = overlay.tick(now: 3.0, underlying: .running, timeoutSeconds: 3.0)
        XCTAssertTrue(overlay.timeoutActive)
    ...
    func testTimeoutOverlayDisabledWhenSecondsZero() {
        var overlay = CodexTimeoutOverlay(now: 0)
        _ = overlay.tick(now: 3.0, underlying: .running, timeoutSeconds: 3.0)
        XCTAssertTrue(overlay.timeoutActive)
